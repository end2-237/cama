...
export default function PersonalCalendarDrawer({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  return (
    <>
      ...
      {/* Drawer */}
      <div
        className={`fixed top-0 right-0 bottom-0 z-[160] w-full lg:w-[78%] bg-white shadow-2xl flex flex-col transition-transform duration-300 ease-out ${
      ...
      >
        ...
        {/* Semaine — grille de colonnes par jour */}
              ...
              <div key={day} className="border border-border bg-white">
                ...
                <div className="divide-y divide-border">
                  {slots.map((s, i) => {
                    ...
                    return (
                      <div key={i} className="px-3 py-2.5 flex items-start gap-2.5 hover:bg-surface transition-colors">
                        ...
                        <div className="min-w-0 flex-1">
                          <div className="flex items-center gap-1.5 flex-wrap">
                            <p className="text-[10px] font-bold text-ink flex items-center gap-1">
                              <Clock className="w-2.5 h-2.5 text-subtle" /> {s.time}
                            </p>
                          ...
                          <p className="text-[9px] text-muted flex items-center gap-2 mt-0.5">
                            <span className="flex items-center gap-1"><BookOpen className="w-2.5 h-2.5" /> {s.ue}</span>
                  ...
                  {slots.length === 0 && (
                    <p className="px-3 py-3 text-[11px] text-subtle italic">Aucune séance — travail personnel.</p>
                  )}
                </div>
              </div>
            ))}
