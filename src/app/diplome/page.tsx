...
import { ArrowLeft, Download, GraduationCap, ShieldCheck, Loader2, Printer, FileCheck2 } from "lucide-react";
import { useAuth } from "@/context/AuthContext";
import { fetchDeliberations, type DelibWithMeta } from "@/lib/exams";
import { fetchStudentDiplomas, DIPLOMA_KIND_LABEL, type DBDiploma } from "@/lib/diplomas";
...
export default function DiplomePage() {
  ...
  useEffect(() => {
    if (!user) return;
    (async () => {
      const [all, dips] = await Promise.all([
        fetchDeliberations(),
        fetchStudentDiplomas(user.id),
      ]);
  ...
  return (
    <div className="min-h-screen bg-surface py-10 px-4">
      <div className="max-w-2xl mx-auto">
        ...
        {/* ── Diplômes & attestations émis ── */}
        <section className="mt-8 print:hidden">
          <div className="flex items-center gap-2 border-b-2 border-cama/20 pb-2 mb-4">
            <FileCheck2 className="w-4 h-4 text-cama" />
            <h2 className="text-sm font-bold uppercase tracking-wide text-ink">Diplômes & attestations émis</h2>
          </div>
          {diplomas.length ? (
            <ul className="space-y-3">
              {diplomas.map((d) => (
                <li key={d.id} className={`bg-white border-2 p-4 flex items-center gap-4 ${d.revoked ? "border-red-200 opacity-70" : "border-cama/20"}`}>
                  <div className="border-2 border-ink p-1 bg-white flex-shrink-0">
                    <QrSvg data={d.code} size={64} />
                  </div>
                  <div className="min-w-0 flex-1">
                    <p className="text-sm font-bold text-ink">{DIPLOMA_KIND_LABEL[d.kind]}</p>
                    <p className="text-xs text-muted">
                      {[d.parcours_title, d.level, d.academic_year].filter(Boolean).join(" · ") || "—"}
                    </p>
                    <p className="text-[10px] font-mono text-muted mt-1">{d.code}</p>
                    {d.revoked ? (
                      <p className="text-[11px] font-bold text-red-600 mt-1">Document révoqué — non valable</p>
                    ) : (
                      <Link href={`/verifier/${d.code}`} className="text-xs text-cama font-bold hover:underline">
                        Vérifier — cama.jfn.cm/verifier/{d.code}
                      </Link>
                    )}
                  </div>
                  <button
                    onClick={() => window.print()}
                    className="inline-flex items-center gap-1.5 border-2 border-cama px-3 py-2 text-xs font-semibold text-cama hover:bg-cama-50 transition-colors flex-shrink-0"
                  >
            ...
            <p className="text-sm text-muted italic">Aucun diplôme ou attestation n&apos;a encore été émis par l&apos;administration.</p>
          )}
        </section>
          </>
        )}
